...
public class InstructionTester {
	...
	public void testFORImmediate(){
	    ...
	    machine.setCode(0x78,0);
	    
	    machine.setData(0, 4095);
	    machine.setData(1, 1);
	    machine.setData(2, 3);
	    
	    instr.execute(0x3005, 2);
	    
	    int [] retVal = machine.getData();
	    ...
	    //Memory data matches expected data
	    assertArrayEquals(dataCopy,retVal);
	    
	    //Accumulator is 8
	    assertEquals( "Accumulator is 8",8,machine.getAccum());
	    //Program counter incremented
	    assertEquals("Program counter incremented to 5",5,machine.getPC());
